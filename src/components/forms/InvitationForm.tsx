...
import { saveActivity, sendInvitation } from "@/lib/queries";
...
const InvitationForm: React.FC<SendInvitationProps> = ({ agencyId }) => {
  ...
  const onSubmit = async (values: z.infer<typeof userDataSchema>) => {
    try {
      const res = await sendInvitation(values.email, values.role, agencyId);
      if (res)
        await saveActivity({
          agencyId: agencyId,
          desc: `Invited ${res.email}`,
          subAccId: undefined,
        });
      ToastNotify({
        title: "Success",
        msg: "Created and sent invitation",
      });
    } catch (error) {
      console.log(error);
      ToastNotify({
